...
import { searchEntities, getEntities } from './tag.reducer';
  ...
  return (
    <div>
      <h2 id="tag-heading" data-cy="TagHeading">
        <Translate contentKey="litApp.socialTag.home.title">Tags</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="litApp.socialTag.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="litApp.socialTag.home.createLabel">Create new Tag</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('litApp.socialTag.home.search')}
                />
        ...
        {tagList && tagList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="litApp.socialTag.id">ID</Translate>
                </th>
                <th>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {tag.posts
                      ? tag.posts.map((val, j) => (
                          <span key={j}>
                            <Link to={`post/${val.id}`}>{val.id}</Link>
                            {j === tag.posts.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>
                    {tag.comments
                      ? tag.comments.map((val, j) => (
                          <span key={j}>
                            <Link to={`comment/${val.id}`}>{val.id}</Link>
                            {j === tag.comments.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>
                    {tag.replies
                      ? tag.replies.map((val, j) => (
                          <span key={j}>
                            <Link to={`reply/${val.id}`}>{val.id}</Link>
                            {j === tag.replies.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="litApp.socialTag.home.notFound">No Tags found</Translate>
            </div>
          )
        )}
      </div>
